...
describe('platform access guard', () => {
  ...
  it('fails closed when the platform administrator store is unavailable', async () => {
    const deps: PlatformAccessDependencies = {
      getUserId: vi.fn().mockResolvedValue('authenticated-profile-id'),
      findPlatformAdmin: vi.fn().mockRejectedValue(new Error('platform_admins unavailable')),
    }

    await expect(getPlatformAccess('control_center.access', deps)).rejects.toThrow(
      'platform_admins unavailable'
    )
  })
